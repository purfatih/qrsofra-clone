...
const ProductValidationSchema = Yup.object({
  ...
  branches: Yup.array()
    ...
    .required("En az bir şube seçiniz!"),
  categories: Yup.array()
    .min(
      1,
      "En az bir kategori seçiniz, kategoriniz yoksa 'Kategoriler' kısmından oluşturabilirsiniz.",
    )
    .required("En az bir kategori seçiniz!"),
});
const ExtraProductValidationSchema = Yup.object({
  name: Yup.string()
    .required("Yan ürün adı en az 2 karakter olmalıdır")
    .min(2, "Yan ürün adı en az 2 karakter olmalıdır"),
});
export {
